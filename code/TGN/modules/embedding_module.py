...
class GraphEmbedding(EmbeddingModule):
  ...
  def compute_embedding(self, memory, source_nodes, timestamps, n_layers, n_neighbors=20, time_diffs=None,
    ...
    if n_layers == 0:
      ...
      sum_of_rows = torch.linalg.norm(source_embedding, dim = 1)
      normalized_source_embedding = source_embedding / sum_of_rows[:, np.newaxis]
      return normalized_source_embedding

  def compute_embedding_grad(self, memory, source_nodes, timestamps, n_layers, nodes_features, nodes_memory, neigh_features,
    ...
    """if self.use_memory:
      source_node_features = memory[source_nodes, :] + source_node_features"""
    if n_layers == 0:
      source_node_features = neigh_memory + neigh_features
    if n_layers == 1:
      source_node_features = nodes_features + nodes_memory
    if n_layers == 0:
      return source_node_features
    else:
      ...
      neighbors = neighbors.flatten()
      neighbor_embeddings = self.compute_embedding_grad(memory,
                                                   neighbors,
                                                   np.repeat(timestamps, n_neighbors),
                                                   n_layers=n_layers - 1,
